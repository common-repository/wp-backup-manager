...
class UpdraftPlus_Options {
	...
	public static function add_admin_pages() {
		...
		add_submenu_page('options-general.php', 'WP Backup Manager', __('WP Backup Manager','updraftplus'), apply_filters('option_page_capability_updraft-options-group', 'manage_options'), "wp_backup_manager", array($updraftplus_admin, "settings_output"));
	}
	...
	public static function options_form_begin($settings_fields = 'updraft-options-group', $allow_autocomplete = true, $get_params = array()) {
		...
		if ($settings_fields) {
			...
			if (substr($referer, -19, 19) == 'options-general.php' && false !== strpos($_SERVER['REQUEST_URI'], '?')) $referer .= '?page=wp_backup_manager';
	...
	public static function admin_init() {
		...
		global $pagenow;
		if (is_multisite() && $pagenow == 'options-general.php' && isset($_REQUEST['page']) && 'wp_backup_manager' == substr($_REQUEST['page'], 0, 17)) {
	...
	public static function show_admin_warning_multisite() {
		global $updraftplus_admin;
		$updraftplus_admin->show_admin_warning('<strong>'.__('WP Backup Manager warning:', 'updraftplus').'</strong> '.__('This is a WordPress multi-site (a.k.a. network) installation.', 'updraftplus').' '.__('Without upgrading, WP Backup Manager allows <strong>every</strong> blog admin who can modify plugin settings to back up (and hence access the data, including passwords, from) and restore (including with customised modifications, e.g. changed passwords) <strong>the entire network</strong>.', 'updraftplus').' '.__('(This applies to all WordPress backup plugins unless they have been explicitly coded for multisite compatibility).', 'updraftplus'), 'error');
	}
