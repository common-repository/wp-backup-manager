...
function updraft_popuplog(backup_nonce) { 
		...
		updraft_send_command('get_log', backup_nonce, function(resp) {
			...
			var download_url = '?page=wp_backup_manager&action=downloadlog&force_download=1&updraftplus_backup_nonce='+resp.nonce;
		...
		uploader.bind('FileUploaded', function(up, file, response) {
			...
			if (response.status == '200') {
				...
				if (response.response.substring(0,6) == 'ERROR:') {
					...
					jQuery('#' + file.id).append(updraftlion.uploaded+' <a href="?page=wp_backup_manager&action=downloadfile&updraftplus_file='+bkey+'&decrypt_key='+encodeURIComponent(jQuery('#updraftplus_db_decrypt').val())+'">'+updraftlion.followlink+'</a> '+updraftlion.thiskey+' '+jQuery('#updraftplus_db_decrypt').val().replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"));
				} else {
