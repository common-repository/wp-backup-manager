...
// 				$next_scheduled_backup_database = date_i18n('D, F j, Y H:i', $next_scheduled_backup_database);
	...
	private function admin_init() {
		...
		if ($pagenow != UpdraftPlus_Options::admin_page() || empty($_REQUEST['page']) || 'wp_backup_manager' != $_REQUEST['page']) return;
...
// TODO: Deactivate the auth code whilst the operation is underway
	...
	public function wp_before_admin_bar_render() {
		...
		$args = array(
			...
			'title' => apply_filters('updraftplus_admin_node_title', 'WP Backup Manager')
		);
		...
		$args = array(
			...
			'href' => $option_location.'?page=wp_backup_manager&tab=status'
		);
		$wp_admin_bar->add_node($args);
		...
		$args = array(
			...
			'parent' => 'updraft_admin_node',
			'href' => $option_location.'?page=wp_backup_manager&tab=backups'
		);
		$wp_admin_bar->add_node($args);
		...
		$args = array(
			...
			'parent' => 'updraft_admin_node',
			'href' => $option_location.'?page=wp_backup_manager&tab=settings'
		);
		$wp_admin_bar->add_node($args);
		...
		$args = array(
			...
			'parent' => 'updraft_admin_node',
			'href' => $option_location.'?page=wp_backup_manager&tab=expert'
		);
		$wp_admin_bar->add_node($args);
...
// 			'resetting' => __('Resetting...', 'updraftplus'),
			...
			'browse_download_link' => apply_filters('updraftplus_browse_download_link', ''),
			...
			'updraft_settings_url' => UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager', 
	...
	public function admin_head() {
		...
		if ($pagenow != UpdraftPlus_Options::admin_page() || !isset($_REQUEST['page']) || 'wp_backup_manager' != $_REQUEST['page'] || !UpdraftPlus_Options::user_can_manage()) return;
...
// 		}
	...
	public function plugin_action_links($links, $file) {
		if (is_array($links) && $file == 'wp_backup_manager/updraftplus.php'){
			$settings_link = '<a href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager">'.__("Settings", "updraftplus").'</a>';
...
// 			array_unshift($links, $settings_link);
	...
	public function show_admin_warning_multiple_storage_options() {
		$this->show_admin_warning('<strong>WP Backup Manager:</strong> '.__('An error occurred when fetching storage module options: ', 'updraftplus').htmlspecialchars($this->storage_module_option_errors), 'error');
	}
	...
	public function show_admin_nosettings_warning() {
		$this->show_admin_warning('<strong>'.__('Welcome to WP Backup Manager!', 'updraftplus').'</strong> '.__('To make a backup, just press the Backup Now button.', 'updraftplus').' <a href="#" id="updraft-navtab-settings2">'.__('To change any of the default settings of what is backed up, to configure scheduled backups, to send your backups to remote storage (recommended), and more, go to the settings tab.', 'updraftplus').'</a>', 'updated notice is-dismissible');
	}
	...
	public function show_admin_warning_dropbox() {
		$this->show_admin_warning('<strong>'.__('UpdraftPlus notice:', 'updraftplus').'</strong> <a class="updraft_authlink" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=updraftmethod-dropbox-auth&updraftplus_dropboxauth=doit">'.sprintf(__('Follow this link to authorize access to your %s account (you will not be able to back up to %s without it).', 'updraftplus'), 'Dropbox', 'Dropbox').'</a>', 'updated updraft_authenticate_dropbox');
	}

	public function show_admin_warning_onedrive() {
	$this->show_admin_warning('<strong>'.__('UpdraftPlus notice:', 'updraftplus').'</strong> <a class="updraft_authlink" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=updraftmethod-onedrive-auth&updraftplus_onedriveauth=doit">'.sprintf(__('Follow this link to authorize access to your %s account (you will not be able to back up to %s without it).', 'updraftplus'), 'OneDrive', 'OneDrive').'</a>', 'updated updraft_authenticate_onedrive');
	}
	...
	public function show_admin_warning_googledrive() {
		$this->show_admin_warning('<strong>'.__('UpdraftPlus notice:', 'updraftplus').'</strong> <a class="updraft_authlink" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=updraftmethod-googledrive-auth&updraftplus_googleauth=doit">'.sprintf(__('Follow this link to authorize access to your %s account (you will not be able to back up to %s without it).', 'updraftplus'), 'Google Drive', 'Google Drive').'</a>', 'updated updraft_authenticate_googledrive');
	}

	public function show_admin_warning_googlecloud() {
		$this->show_admin_warning('<strong>'.__('UpdraftPlus notice:', 'updraftplus').'</strong> <a class="updraft_authlink" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=updraftmethod-googlecloud-auth&updraftplus_googleauth=doit">'.sprintf(__('Follow this link to authorize access to your %s account (you will not be able to back up to %s without it).', 'updraftplus'), 'Google Cloud', 'Google Cloud').'</a>', 'updated updraft_authenticate_googlecloud');
	}
...
// TODO: Deal with the case of some of the work having been deferred
				...
				echo '<strong>'.__('Actions', 'updraftplus').':</strong> <a href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&updraft_restore_success='.$s_val.'&pval='.$pval.'">'.__('Return to UpdraftPlus Configuration', 'updraftplus').'</a>';
				return;
				...
				echo '<strong>'.__('Actions', 'updraftplus').':</strong> <a href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager">'.__('Return to UpdraftPlus Configuration', 'updraftplus').'</a>';
				return;
		...
		if (isset($_GET['action']) && $_GET['action'] == 'updraft_create_backup_dir' && isset($_GET['nonce']) && wp_verify_nonce($_GET['nonce'], 'create_backup_dir')) {
			...
			echo '<b>'.__('Actions', 'updraftplus').':</b> <a href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager">'.__('Return to UpdraftPlus Configuration', 'updraftplus').'</a>';
			return;
		}
...
// 			$this->print_restore_in_progress_box_if_needed();
	...
	public function show_admin_restore_in_progress_notice() {
		...
		<form method="post" action="<?php echo UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager'; ?>">
...
// 		$ret .= '<p>'.__('Does nothing happen when you attempt backups?', 'updraftplus').' <a href="https://updraftplus.com/faqs/my-scheduled-backups-and-pressing-backup-now-does-nothing-however-pressing-debug-backup-does-produce-a-backup/">'.__('Go here for help.', 'updraftplus').'</a></p>';
	...
	private function backup_now_widgetry() {
		...
		$free_ret = '<em>'.__('All WordPress tables will be backed up.', 'updraftplus').' </em>';
	...
	private function most_recently_modified_log_link() {
		...
		?>
			<a href="?page=wp_backup_manager&amp;action=downloadlatestmodlog&amp;wpnonce=<?php echo wp_create_nonce('updraftplus_download') ?>" <?php if (!$mod_time) echo 'style="display:none;"'; ?> class="updraft-log-link" onclick="event.preventDefault(); updraft_popuplog('');"><?php _e('Download most recently modified log file', 'updraftplus');?></a>
		<?php
	}
...
// 		if (0 == $found_jobs) $ret .= '<p><em>'.__('(None)', 'updraftplus').'</em></p>';
	...
	private function print_active_job($job_id, $is_oneshot = false, $time = false, $next_resumption = false) {
		...
		$ret .= '- <a data-jobid="'.$job_id.'" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=downloadlog&updraftplus_backup_nonce='.$job_id.'" class="updraft-log-link">'.__('show log', 'updraftplus').'</a>';
	...
	private function delete_old_dirs_go($show_return = true) {
		...
		if ($show_return) echo '<b>'.__('Actions', 'updraftplus').':</b> <a href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager">'.__('Return to UpdraftPlus Configuration', 'updraftplus').'</a>';
	}

	//deletes the -old directories that are created when a backup is restored.
	private function delete_old_dirs() {
		global $wp_filesystem, $updraftplus;
		$credentials = request_filesystem_credentials(wp_nonce_url(UpdraftPlus_Options::admin_page_url()."?page=wp_backup_manager&action=updraft_delete_old_dirs", 'updraftplus-credentialtest-nonce')); 
...
// 		$ret2 = $this->delete_old_dirs_dir($wp_filesystem->abspath());
	...
	private function create_backup_dir() {
		...
		if (false === ($credentials = request_filesystem_credentials(UpdraftPlus_Options::admin_page().'?page=wp_backup_manager&action=updraft_create_backup_dir&nonce='.wp_create_nonce('create_backup_dir')))) {
			return false;
		}

		if ( ! WP_Filesystem($credentials) ) {
			// our credentials were no good, ask the user for them again
			request_filesystem_credentials(UpdraftPlus_Options::admin_page().'?page=wp_backup_manager&action=updraft_create_backup_dir&nonce='.wp_create_nonce('create_backup_dir'), '', true);
			return false;
		}
...
// 				$inc_time = date_i18n('D, F j, Y H:i', $updraft_last_backup['backup_time_incremental']);
			...
			if (!empty($updraft_last_backup['backup_nonce'])) {
				...
				if (is_readable($potential_log_file)) $last_backup_text .= "<a href=\"?page=wp_backup_manager&action=downloadlog&updraftplus_backup_nonce=".$updraft_last_backup['backup_nonce']."\" class=\"updraft-log-link\" onclick=\"event.preventDefault(); updraft_popuplog('".$updraft_last_backup['backup_nonce']."');\">".__('Download log file', 'updraftplus')."</a>";
			}
	...
	public function really_writable_message($really_is_writable, $updraft_dir){
		if ($really_is_writable) {
			...
			$dir_info .= '<span class="updraft-directory-not-writable-blurb"><span class="directory-permissions"><a class="updraft_create_backup_dir" href="'.UpdraftPlus_Options::admin_page_url().'?page=wp_backup_manager&action=updraft_create_backup_dir&nonce='.wp_create_nonce('create_backup_dir').'">'.__('Follow this link to attempt to create the directory and set the permissions', 'updraftplus').'</a></span>, '.__('or, to reset this option', 'updraftplus').' <a href="#" class="updraft_backup_dir_reset">'.__('click here', 'updraftplus').'</a>. '.__('If that is unsuccessful check the permissions on your server or change it to another directory that is writable by your web server process.', 'updraftplus').'</span>';
		}
		return $dir_info;
	}
...
// 			$lt = esc_attr(__('View Log', 'updraftplus'));
			...
			$url = esc_attr(UpdraftPlus_Options::admin_page()."?page=wp_backup_manager&action=downloadlog&amp;updraftplus_backup_nonce=$nval");
			$ret .= <<<ENDHERE
...
// 			return str_replace(' ', '&nbsp;', '('.__('No backup log)', 'updraftplus').')');
	...
	private function restore_backup($timestamp, $continuation_data = null) {
		...
		$credentials = request_filesystem_credentials(UpdraftPlus_Options::admin_page()."?page=wp_backup_manager&action=updraft_restore&backup_timestamp=$timestamp", '', false, false, $extra_fields);
		WP_Filesystem($credentials);
		if ( $wp_filesystem->errors->get_error_code() ) { 
